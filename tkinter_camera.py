...
# label = Label(root, text="Hello, world!")  # 创建一个Label控件，指定其父控件位创建的根窗口，并在Label上显示 hello worl!
# label.pack()  # 显示label控件，pack方法会让label显示，并根据文本内容自动调节大小
#
# bm = PhotoImage(file='/home/xia/working_tools/1.png')
...
#
# root=Tk() # 这句必须先于 ImageTk.PhotoImage 执行。
# s='/home/xia/working_tools/1.png' # jpg图片文件名 和 路径。
# im=Image.open(s)
# tkimg=ImageTk.PhotoImage(im) # 执行此函数之前， Tk() 必须已经实例化。
# l=Label(root,image=tkimg)
# l.pack()
# root.mainloop()
